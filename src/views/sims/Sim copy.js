...
const Sim = () => {
  ...
  const [actives, setActives] = useState(['non', 'oui'])
  ...
  const columns = [
    ...
    { title: 'CODE', data: 'simcode' },
    {
      ...
      render: (data, type, row) => {
        const btnShow = row.demande
          ? `<a class="btn btn-outline-secondary me-1 table-btn tableBtnShowRelationship" href="#" data-id="${row.id}" data-relation="demande">${row.demande.dmdcode}</a>`
          : ''
    ...
    },
    {
      ...
      render: (data, type, row) => {
        ...
        const btnDemande = row.demande
          ? `<a class="btn btn-outline-secondary tableActionBtn tableActionBtnDissocierItem" href="#" data-numero="${row.simnumero}" data-demande="${row.demande && row.demande.id}" data-codedemande="${row.demande && row.demande.dmdcode}" title="Dissocier de la demande"><i class="fa fa-close" aria-hidden="true"></i></a>`
          : `<a class="btn btn-outline-secondary tableActionBtn tableActionBtnDemandeItem" href="#" data-sim="${row.id}" data-numero="${row.simnumero}" title="Attribuer à une demande"><i class="fa fa-paper-plane" aria-hidden="true"></i></a>`

        return `<div class="d-flex">${btnEdit + btnDelete + btnDemande}</div>`
      },
    },
  ]
  ...
  const fetchGet = async () => {
    try {
      ...
      setData(response)
      setSims(response.filter((item) => item.demande === null))
    } catch (err) {
      setError(err)
    } finally {
      setLoading(false)
  ...
  const fetchGetDemande = async () => {
    await getItemBy('demandes/assignable')
      .then((response) => {
        if (response.success) {
          if (response.data) {
        ...
        layout: {
          top1Start: {
            buttons: [
              ...
              {
                text: '<i class="fa fa-paper-plane me-1" aria-hidden="true"></i>Attribuer',
                className: 'dt-btn datatable-button rounded dt-btnAttribute btnAttribute ms-2',
                enabled: demandes && demandes.length > 0 ? true : false,
                action: () => {
  ...
  $('#myTable tbody').on('click', '.tableActionBtnEditItem', async function (e) {
    ...
    if (createFormRef.current && createFormBtnLaunchRef.current) {
      if (response) {
        ...
        $('#code').val(response.simcode)
        $('#numero').val(response.simnumero)
        $('#dateactivation').val(response.simdateactivation)
        $('#dateremise').val(response.simdateremise)
        $('#datesuspension').val(response.simdatesuspension)
        $('#dateretrait').val(response.simdateretrait)
        $('input[name="perdue"][value="' + response.simperdue + '"]').prop('checked', true)
        $('#commentaire').val(response.simcommentaire)
        createFormBtnLaunchRef.current.click()
      }
    }
  })
  ...
  const handleSubmitImportForm = async (e) => {
    ...
    if (importFormRef.current && importFormBtnCloseRef.current) {
      ...
      const response = await createItem('sims/import', {
  ...
  const handleSubmitAttributeForm = async (e) => {
    ...
    if (attributeFormRef.current && attributeFormBtnCloseRef.current) {
      ...
      if (attribute === 'unique') {
        ...
        formValues.sim = attributeFormRef.current.getAttribute('data-sim')
        formValues.demande = attributeFormRef.current.getAttribute('data-demande')
      }
      const response = await createItem('demande/attribuer/sim', formValues)
      if (response.success) {
        if (attributeFormRef.current && attributeFormBtnCloseRef.current) {
  ...
  // === Dissocier form
  const handleCancelDissocierForm = () => {
    if (importFormRef.current && importFormBtnCloseRef.current) {
      importFormRef.current.setAttribute('form-action', '')
      importFormRef.current.setAttribute('target-id', '')
      setExcelData([])
  ...
  const handleSubmitDissocierForm = async (e) => {
    e.preventDefault()
    // récupération des données de la liste
    if (attributeFormRef.current && attributeFormBtnCloseRef.current) {
      const formData = new FormData(attributeFormRef.current)
      const formValues = Object.fromEntries(formData)
      const data = excelData.filter((_, index) => index > 0)
      formValues.demande = dissocierFormRef.current.getAttribute('data-demande')
      console.log(formValues)
      const response = await createItem('demande/dissocier/sim', formValues)
      if (response.success) {
        if (dissocierFormRef.current && dissocierFormBtnCloseRef.current) {
          dissocierFormBtnCloseRef.current.click()
        }
      }
      setIndexAlert(response)
      fetchGet()
    ...
    <div className="container">
      <div className="row">
        <div className="col-md-12 offset-md-0">
              ...
              <div className="modal-dialog modal-md modal-dialog-scrollable">
                <div className="modal-content">
                  ...
                  <div className="modal-footer border-0 py-0">
                    <button
                      type="submit"
                      className="btn custom-btn-primary text-white createModalBtnSave"
                    >
              ...
              <div className="modal-dialog modal-lg modal-dialog-scrollable">
                <div className="modal-content">
                  ...
                  <div className="modal-body">
                    ...
                    <div className="card">
                      <div className="card-body">
                        {/* Cartes SIM & Demandes */}
                        <div
                          id="attributeFormHeader"
                          className="text-light p-1 fw-bolder text-center"
                  ...
                  <div className="modal-footer border-0 py-0">
                    <button
                      type="submit"
                      className="btn custom-btn-success text-white attributeModalBtnSave"
                    >
                      <i className="fa fa-check me-1" aria-hidden="true"></i>
                      Valider
                    </button>
                    <button
                      type="button"
                      className="btn btn-secondary attributeModalBtnCancel"
                      data-bs-dismiss="modal"
          ...
          {/* Dissocier form */}
          <form
            ref={dissocierFormRef}
            onSubmit={handleSubmitDissocierForm}
            method="POST"
            encType=""
            form-action="dissocier"
            target-type=""
            data-demande=""
          >
            <button
              ref={dissocierFormBtnLaunchRef}
              type="button"
              className="btn btn-primary d-none"
              data-bs-toggle="modal"
              data-bs-target="#dissocierModal"
            >
              <i className="fa fa-plus me-2" aria-hidden="true"></i>Launch demo static modal
            </button>
            {/* <!-- Modal --> */}
            <div
              ...
              data-bs-keyboard="false"
              tabIndex="-1"
              aria-labelledby="dissocierModal"
              aria-hidden="true"
            >
              <div className="modal-dialog modal-lg modal-dialog-scrollable">
                <div className="modal-content">
                  <div className="modal-header py-1 bg-primary">
                    <h5 className="modal-title  fw-bold text-light" id="dissocierModalLabel">
                      <i className="fa fa-paper-plane me-1" aria-hidden="true"></i>Dissocier une
                      carte SIM
                    </h5>
                    <button
                      ref={dissocierFormBtnCloseRef}
                      type="button"
                      className="btn-close d-none"
                      data-bs-dismiss="modal"
                      aria-label="Close"
                  ...
                  <div className="modal-footer border-0 py-0">
                    <button
                      type="submit"
                      className="btn custom-btn-success text-white dissocierModalBtnSave"
                    >
                      <i className="fa fa-check me-1" aria-hidden="true"></i>
                      Valider
                    </button>
                    <button
                      type="button"
                      className="btn btn-secondary dissocierModalBtnCancel"
                      data-bs-dismiss="modal"
                      onClick={handleCancelDissocierForm}
                    >
                      <i className="fa fa-close me-1" aria-hidden="true"></i>Fermer
                    </button>
                  </div>
