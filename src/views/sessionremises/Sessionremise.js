...
const Sessiondemande = () => {
  ...
  const apiResource = {
    get: 'sessionremises',
    show: 'sessionremises/:id',
    create: 'sessionremises',
    update: 'sessionremises/:id',
    delete: 'sessionremises/:id',
  }
    ...
    await getItem(apiResource.show.replace(':id', id)).then((response) => {
      if (response.success) {
        if (createFormRef.current && createFormBtnLaunchRef.current) {
          ...
          $('#datedebut').val(r.serdatedebut)
          $('#datefin').val(r.serdatefin)
          $('input[name="active"][value="' + r.seractive + '"]').prop('checked', true)
          $('#commentaire').val(r.sercommentaire)
          createFormBtnLaunchRef.current.click()
        }
      } else {
        //
